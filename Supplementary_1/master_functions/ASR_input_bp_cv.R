##### 2.9.22
# this version does not include a SnpChip - just QTL for causal variant RRBLUP approach 
...
ASR_input_CV_bp <- function(founder_data, QTL_n){
  ### STEP 0 
  ...
  ##### Random QTL ##### 
    ...
    genotype <- cbind(founder_data_filter_df[,4], founder_data_filter_df[,17:18]) 
    ...
    for(i in 1:length(split_Chr_haplotype)){
      # remove chr column
      snps <- split_Chr_haplotype[[i]][,2:3]
      # transpose
    ...
    QTL_per_chr <<- info_ASR_input_R %>% filter(Type == "QTL") %>% dplyr::select(Chr) %>% table() %>% as.data.frame()
    dv_loci <<- info_ASR_input_R %>% filter(Type == "QTL")
