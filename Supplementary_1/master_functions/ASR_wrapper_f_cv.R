...
ASR_wrapper_CV <- function(type, map_scale, peri_scale, genMap_input, haplotypes_input, info_ASR_input, QTL_effects, QTL_effect_ratio,
                        heritability, legend){
  ######### Wrapper function for creating ASR founder haplotypes, setting simulation parameters, and modeling the breeding program
  ...
  if(peri_scale > 1){
    ...
    # call DV_scale_map
    DV_scale_map(genMap_change = my_genMap_change, DV_scale = peri_scale, BP = FALSE)
  ...
  gsPop <- TP 
  #founder_AF = pullSegSiteGeno(gsPop, simParam = SP2) %>% as.data.table() # used for tracking DV AF beginning to end
  ...
  for (cycle in 1:nGScyc){
    ...
    ans = RRBLUP(TP, simParam=SP2, useQtl = TRUE)
    DH = setEBV(DH, ans, simParam=SP2)
    #Evaluate accuracy
    allAcuracy <- c(allAcuracy, cor(gv(DH), ebv(DH)))
    # Evaluate Bulmer effect and fixed QTL
